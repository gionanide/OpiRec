...
colorama.init()#INITIALIZE DEBBUGING
...
def minibatch_training():
	...
	''' IF YOU WANT TO INITIALIZE THE WEIGHTS USING ORTHOGONALITY TRICK
	...
	'''
        ...
        first = next(iter(train_dataset_loader))#XXX IF YOU WANT TO OVERFIT A SAMPLE AND TRY NEW ARCHITECTURES
        #with GuruMeditation():
        for epoch in range(epochs):
                ...
                with torch.no_grad():
                        ...
                        role='Testing'
                        rating_loss_test, review_loss = validation(recommender_encoder,recommender_decoder,epoch,role)
                        #role='Training'
                        #rouge_score_train, rating_loss_train = validation(recommender_encoder,recommender_decoder,epoch,role)
                        
                print('Epoch:',str(epoch+1)+'/'+str(epochs),'Batch:',(str(index)+'/'+str(int(overall_batch))),'Samples',str(batch_size*(index+1))+'/'+str(length),'Output length:',output_length,'----- LEARNING ----- review:',"{0:.4f}".format(sum(overall_review_loss)/len(overall_review_loss)),'rating:',"{0:.4f}".format(sum(overall_rating_loss)/len(overall_rating_loss)),'Time:',"{0:.2f}".format(epoch_time),'----- DEV ----- review:',"{0:.4f}".format(review_loss),'rating',"{0:.4f}".format(rating_loss_test),'teacher_f_r',teacher_forcing_ratio)
                ...
                torch.save(recommender_encoder.state_dict(), '/media/data/gionanide/OpinionRecommendation_working_demo/pytorch_models/new_models/encoder.txt')
                torch.save(recommender_decoder.state_dict(), '/media/data/gionanide/OpinionRecommendation_working_demo/pytorch_models/new_models/decoder.txt')
...
def validation(recommender_encoder,recommender_decoder,epoch,role,deep_memory_calculations):
        #do not build the computational graph
        #with torch.no_grad():
        ...
        #for index, (user_reviews, product_reviews, neighbourhood_reviews, product_ratings, text_reviews, rating_reviews, decoder_inputs) in enumerate([first]):
                ...
                product_ratings = product_ratings.to(device1)
                #run the encoder
                overall_context_vector_encoder, overall_attention_weights, decoder_hidden, deep_memory_calculations = recommender_encoder(user_reviews, product_reviews,deep_memory_calculations)
                #print('index:',index,'calculations',deep_memory_calculations)
                #activations_loss, activations, rating_prediction, repetitiveness_loss_back = encoder_decoder_parallel(user_reviews, product_reviews, output_length, product_ratings, decoder_inputs, mask, teacher_forcing_ratio)
                ...
                for sample in range(activations_loss.shape[0]):
                        ...
                        predicted_str, target_str, rouge_score = evaluation.make_sentence_one_by_one(predicted_sentence, ground_truth, target_reviews_length, tokenizer, role, count_predictions, rating_prediction_item, ground_truth_rating,print_sentence,max_review_length)
                ...
                #break
        #print(sum(overall_rating_loss)/len(overall_rating_loss), sum(overall_review_loss)/len(overall_review_loss), sum(overall_rouge_score)/len(overall_rouge_score))
        
        rating_mean = sum(overall_rating_loss)/len(overall_rating_loss)
        rating_list = []
        for rating in overall_rating_loss:
                rating_list.append((rating-rating_mean)**2)
        rating_std = sum(rating_list)/len(rating_list)

        return sum(overall_rouge_score)/len(overall_rouge_score), rating_mean, rating_std, sum(overall_review_loss)/len(overall_review_loss), n_gram_overlap_predictions, n_gram_overal_groundtruth, deep_memory_calculations,rating_std
