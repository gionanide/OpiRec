...
class Overall_Encoder(torch.nn.Module):
        ...
        def __init__(self, hidden_units, input_dropout, episodes, normalize_product_reviews, number_of_layers, parallel):
                ...
                self.user_lstm = torch.nn.LSTM(300, hidden_units, bidirectional=True, num_layers=self.number_of_layers, dropout=0.2, bias=True, batch_first=True)
                self.product_lstm = torch.nn.LSTM(300, hidden_units, bidirectional=True, num_layers=self.number_of_layers, dropout=0.2, bias=True, batch_first=True)
