...
def minibatch_training():
        ...
        #XXX------> run PARALLEL and DISTRIBUTED
        '''
        ...
        #XXX------> Try a joined network which is PARALLEL and DISTRIBUTED
        '''
        #A joint network
        ...
        pretrained=False
        if(pretrained):
                recommender_encoder.load_state_dict(torch.load('./pytorch_models/new_models/1925_encoder_1_58_0_49.txt'))
                recommender_decoder.load_state_dict(torch.load('./pytorch_models/new_models/1925_decoder_1_58_0_49.txt'))
        ...
        teacher_forcing_ratio = 0 #scheduled_sampling(5,0)        
        teacher_forcing_ratio_decay = 0
        
        
        ''' DEFINE THE DATASET '''        
        train_dataset = dataset_loader.Dataset_yelp('./data/training_set.pkl', transform=None) #output_space = 3519        
        test_dataset = dataset_loader.Dataset_yelp('./data/testing_set.pkl', transform=None) #output_space = 3519
        ...
        for epoch in range(epochs):
                ...
                print('Ep:',str(epoch+1)+'/'+str(epochs),'Out_length:',output_length,'--- LEARNING --- review:',"{0:.4f}".format(sum(overall_review_loss)/len(overall_review_loss)),'rating:',"{0:.4f}".format(sum(overall_rating_loss)/len(overall_rating_loss)),'Time:',"{0:.2f}".format(epoch_time)) 
                ...
                #torch.save(recommender_encoder.state_dict(), './pytorch_models/new_models/encoder.txt')
                #torch.save(recommender_decoder.state_dict(), './pytorch_models/new_models/decoder.txt')
