...
class DMN(torch.nn.Module):
        ...
        def __init__(self, hidden_units, output_space, dropout_after_linear, dropout_lstm_output, number_of_layers, parallel, device1, pad_token, eos_token, sos_token, beam_decode, language_output, embedding_d):
                ...
                self.embedding_layer = torch.nn.Embedding(len(language_output.word2index), self.embedding_d, padding_idx=0)
                ...
                self.attention_combine = torch.nn.Linear(self.hidden_units*2, hidden_units)
        ...
        def forward(self, overall_context_vector_encoder, decoder_hidden, decoder_inputs, target_length,  overall_attention_weights, product_rating, mask, teacher_forcing_ratio):   
                ...
                for timestep in range(target_length):
                        ...
                        if ( (random_number <= teacher_forcing_ratio) or (timestep==0) ):
                                ...
                                #print("0",decoder_inputs)
                                #print("1",decoder_inputs[:,timestep])
                                #print("2",decoder_inputs[:,timestep].shape)
                                teacher_forcing_input = self.embedding_layer(decoder_inputs[:,timestep])
                                #print("3",teacher_forcing_input.shape)  
                                
                        else:
                                teacher_forcing_input = torch.empty(classify.shape[0], self.embedding_d)
                                for index_activation, activation in enumerate(classify):
                                    #word_id = torch.argmax(activation).item()                  
                                    teacher_forcing_input[index_activation] = self.embedding_layer(torch.argmax(activation))
                                    #print("5",teacher_forcing_input[index_activation].shape)  
                                #print("6",teacher_forcing_input.shape)        
                                teacher_forcing_input = teacher_forcing_input.to(self.device1)# [b, 1, 300]
                                
                                #nan = torch.isnan(teacher_forcing_input)
                                #print('teacher_forcing_input_inpuuuuuuuuuuuuuuut',(nan!=0).nonzero())
                        teacher_forcing_input = teacher_forcing_input.unsqueeze(1)
                        #print("7",teacher_forcing_input.shape) 
                        ...
                        #combine them
                        #print(teacher_forcing_result.shape)
                ...
                rating_prediction = (torch.bmm(overall_attention_weights,product_rating) + torch.tanh(self.rating_prediction(rating_overall)) )              
