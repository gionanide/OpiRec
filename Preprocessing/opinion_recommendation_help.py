...
def make_user_product_array(data):
	...
	for review in range(len(data)):
		...
		#print('User: ',user,'rate business: ',business,'with rate: ',rating)
		...
		#and another one dictionary with all the businesses and one index point at them, which is their position in the matrix that we are going to initialize
	...
	#f_null = open("user_business_matrix_dataframe_withNULLentries.txt","w+")

	#-------------------------------------------------------------------------------> just to check the procedure
	#print user_business_matrix.shape
	#print sorted(dict_businesses.items(), key=operator.itemgetter(1))
	...
	#iterate the numpy array to capture the null values as well
	#for index, value in np.ndenumerate(user_business_matrix):
	
	#        f_null_newrow = str(index[0])+','+str(index[1])+','+str(value)
	#        f_null.write(f_null_newrow)
	#        f_null.write('\n')
...
def cosine_dist(matrix1, neigbours, adjust):
        ...
        for row in range(len(matrix1)):
                ...
                #apply one to the main diagonal to denote that it is the similarity of every row with itself, and this can mess up our job where we are seeking for the minimum distance
                distance_matrix[row,row] = 1
        ...
        '''
        #-----------------------------------------------------------------------------------------------> Memory problems, with this method we read the array (Gb)
...
def makeBusinessUserRecords():
        ...
        #just checking
        #for ID in dict_business_user:
        
        #        print(ID,len(dict_business_user[ID]))
...
def makeNNfeedFormat():
        ...
        with open('/media/data/gionanide/OpinionRecommendation/Proceedings/mapping_business.txt', mode='r') as infile:
                ...
                #print(dict_business_to_columnNumber)
        ...
        with open('/media/data/gionanide/OpinionRecommendation/Training-Testing_sets/user_neigbours_90-10.txt',mode='r') as infile:
                ...
                #because the row_index represent the row_number with is the userId after mapping it with the largeId, (dict_user dictionary is keeping this mapping)
                row_index = 0
                
                dict_user_neighbourhood={}
                
                
                #we have a file that the row denotes the user and evry row contains the neigbourhood of every user, so we use the previous dictionary to make the mapping
                for line in infile:
        ...
        with open('/media/data/gionanide/OpinionRecommendation/Training-Testing_sets/TRAINING_uid-iid-rgt_90-10.txt',mode='r') as infile:
                ...
                #print(dict_user_training)
                #print(('76211','8673') in dict_user_training)
        ...
        #read the file as dictionary, same with the above dictionary, but contains the embeddings of the reviews
        #users_business_reviews_embeddings = open('/media/data/gionanide/OpinionRecommendation/Training-Testing_sets/all_businesses_users_reviews_embeddings.txt','r').read()        
        #dict_users_business_reviews_embeddings = eval(users_business_reviews_embeddings) 
        #print('End reading dictionaries \n')
        ...
        for review in range(len(data)):
                ...
                #print((dict_user_to_rowNumber[data.iloc[review]['user_id'].strip()],dict_business_to_columnNumber[data.iloc[review]['business_id'].strip()]),'\n')
                
                
                #initialize an sample object
                print('\n\n')
                ...
                if(userId_interest in dict_user_neighbourhood):
                        ...
                        #for every of the first 5 neighbours of every user, append all his reviews
                        for neighbour in (dict_user_neighbourhood[userId_interest][:5]):
                        
                        
                                #if(text_review_interest in dict_users_business_reviews[dict_rowNumber_to_user[neighbour]]):
                        
                                #print(neighbour)
                                #print(dict_rowNumber_to_user[neighbour])
                                #print(dict_users_business_reviews[dict_rowNumber_to_user[neighbour]])
                ...
                for review in range(len(data)):
                        ...
                        if(userId == userId_interest):
                                if(not(businessId == businessId_interest)):
                                        #if this two conditions are true we have all the reviews (except the one for the businessId_interest) for the user and we can feed the user model
                        ...
                        #else:
                                #if the user is not the user of interest, and the business is not the business of interest, we check if this user belongs to user's neighbourhood, if it is true we feed the neighbourhood model
                        ...
                        #sample_dict = sample.make_field_dictionary_only_embeddings(user_model_embeddings,product_model_embeddings,user_neighbourhood_model_embeddings)
                        
                #convert the review to the embedding
                #review_embedding = w2v.review2vec(text_review_interest).tolist()
                sample_dict = sample.make_field_dictionary(user_model[:5], product_model[:5], user_neighbourhood_model[:5])
                
                with  open('Feeding_samples_90-10_ready_embeddings/'+str(userId_interest)+'--------'+str(businessId_interest)+'.txt','w+') as outputfile:
                ...
                #print(user_count,product_count,neighbourhood_count)    
