...
'''
def lstm_joint_network(user_training_samples, product_training_samples, neighbourhood_training_samples, training_ground_truth, max_review_length, normalize_reviews, hidden_size, output_vocabulary_size):
        ...
        user_attention = bahdanau_attention.BahdanauAttention(300, False)
        ...
        user_attention_output = keras.layers.Reshape((1, hidden_size))(user_attention_output)
        ...
        neighbourhood_attention = bahdanau_attention.BahdanauAttention(300, False)
        ...
        neighbourhood_attention_output = keras.layers.Reshape((1, hidden_size))(neighbourhood_attention_output)
        ...
        overall_attention = bahdanau_attention.BahdanauAttention(300, True)
        overall_attention_output, overall_attention_weights = overall_attention([product_lstm_h, product_lstm_hidden_sequence, user_attention_output, neighbourhood_attention_output])
        print('Overall attention ------>',overall_attention_output.shape)
        print('Overall attention weights ------>',overall_attention_weights.shape)
        
        
        #--------------------------------------------------------------------------------------------------------------------> Repeat vector
        #repeat vector to make the one hot encoding format
        repeat_vector_vocabulary = keras.layers.RepeatVector(max_review_length, name='repeat_vector_vocabulary')(overall_attention_output)
        print('Repeat vector prepare ------>',repeat_vector_vocabulary.shape)
        
        
        
        #--------------------------------------------------------------------------------------------------------------------> Classify
        dense_output = keras.layers.Dense(output_vocabulary_size, activation='relu',name='classifyyyy')(repeat_vector_vocabulary)
